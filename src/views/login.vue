<template>
  <div class="login-container">
    ...
    <div class="logo-container">
      <img src="@/assets/bz.svg" alt="Logo" style="width:100% ;height:100% "/> <!-- 假设是一个图片logo -->
    </div>
    <div class="form">
      ...
      <el-card shadow="never" class="login-card">
        <el-form ref="form" :model="state.loginForm" :rules="rules">
          <el-form-item prop="username">
            <el-input v-model="state.loginForm.username" placeholder="请输入手机号" />
          </el-form-item>
          <el-form-item prop="password">
            <el-input v-model="state.loginForm.password" show-password placeholder="请输入密码" />
          </el-form-item>
...
const login = async () => {
  try {
    ...
    const response = await log_in(state.loginForm.username,state.loginForm.password,true)
...
.login-container .form {
  ...
  box-shadow: 0 4px 8px rgba(0,0,0,0.1), 0 6px 20px rgba(0,0,0,0.1); /* 更细腻的阴影效果 */
  display: flex;
